...
class Memory extends Adapter
{
    ...
    public function find(Document $collection, array $queries = [], ?int $limit = 25, ?int $offset = null, array $orderAttributes = [], array $orderTypes = [], array $cursor = [], string $forPermission = Database::PERMISSION_READ): array
    {
        $key = $this->key($collection->getId());
        if (! isset($this->data[$key])) {
            throw new NotFoundException('Collection not found');
        ...
        $rows = $this->applyOrdering($rows, $orderAttributes, $orderTypes);
        $rows = $this->applyCursor($rows, $orderAttributes, $orderTypes, $cursor);
     ...
     * @return array<array<string, mixed>>
     */
    protected function applyOrdering(array $rows, array $orderAttributes, array $orderTypes): array
    {
     ...
     * @return array<array<string, mixed>>
     */
    protected function applyCursor(array $rows, array $orderAttributes, array $orderTypes, array $cursor): array
    {
        if (empty($cursor)) {
            return $rows;
        }
