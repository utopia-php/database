...
class FilterTest extends TestCase
{
    ...
    public function testMaxValuesCount(): void
    {
        ...
        $this->assertEquals('Invalid query: Query on attribute has greater than '.$max.' values: integer', $this->validator->getDescription());
    }
    ...
    public function testNotContains(): void
    {
        ...
        $this->assertEquals('Invalid query: NotContains queries require at least one value.', $this->validator->getDescription());
    }
    ...
    public function testNotSearch(): void
    {
        ...
        $this->assertEquals('Invalid query: Cannot query notSearch on attribute "string_array" because it is an array.', $this->validator->getDescription());
    ...
    public function testNotStartsWith(): void
    {
        ...
        $this->assertEquals('Invalid query: Cannot query notStartsWith on attribute "string_array" because it is an array.', $this->validator->getDescription());

        // Test multiple values not allowed
        $query = Query::parse('{"method":"notStartsWith","attribute":"string","values":["prefix1", "prefix2"]}');
        $this->assertFalse($this->validator->isValid([$query]));
        $this->assertEquals('Invalid query: NotStartsWith queries require exactly one value.', $this->validator->getDescription());
    }
    ...
    public function testNotEndsWith(): void
    {
        ...
        $this->assertEquals('Invalid query: Cannot query notEndsWith on attribute "string_array" because it is an array.', $this->validator->getDescription());

        // Test multiple values not allowed
        $query = Query::parse('{"method":"notEndsWith","attribute":"string","values":["suffix1", "suffix2"]}');
        $this->assertFalse($this->validator->isValid([$query]));
        $this->assertEquals('Invalid query: NotEndsWith queries require exactly one value.', $this->validator->getDescription());
    }
    ...
    public function testNotBetween(): void
    {
        ...
        $this->assertEquals('Invalid query: Cannot query notBetween on attribute "integer_array" because it is an array.', $this->validator->getDescription());

        // Test wrong number of values
        $query = Query::parse('{"method":"notBetween","attribute":"integer","values":[10]}');
        $this->assertFalse($this->validator->isValid([$query]));
        $this->assertEquals('Invalid query: NotBetween queries require exactly two values.', $this->validator->getDescription());

        $query = Query::parse('{"method":"notBetween","attribute":"integer","values":[10, 20, 30]}');
        $this->assertFalse($this->validator->isValid([$query]));
        $this->assertEquals('Invalid query: NotBetween queries require exactly two values.', $this->validator->getDescription());
    }
}
