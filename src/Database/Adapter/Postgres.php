...
class Postgres extends SQL
{
    ...
    public function createCollection(string $name, array $attributes = [], array $indexes = []): bool
    {
        ...
        $createTable = $this->onDuplicate !== OnDuplicate::Fail
            ...
            : 'CREATE TABLE';
        $createIndex = $this->onDuplicate !== OnDuplicate::Fail
            ? 'CREATE INDEX IF NOT EXISTS'
            : 'CREATE INDEX';
        $createUniqueIndex = $this->onDuplicate !== OnDuplicate::Fail
        ...
        if ($this->sharedTables) {
            ...
            $collection .= "
				{$createUniqueIndex} \"{$uidIndex}\" ON {$this->getSQLTable($id)} (\"_uid\" COLLATE utf8_ci_ai, \"_tenant\");
            	{$createIndex} \"{$createdIndex}\" ON {$this->getSQLTable($id)} (_tenant, \"_createdAt\");
            	{$createIndex} \"{$updatedIndex}\" ON {$this->getSQLTable($id)} (_tenant, \"_updatedAt\");
            	{$createIndex} \"{$tenantIdIndex}\" ON {$this->getSQLTable($id)} (_tenant, _id);
			";
        } else {
            ...
            $collection .= "
				{$createUniqueIndex} \"{$uidIndex}\" ON {$this->getSQLTable($id)} (\"_uid\" COLLATE utf8_ci_ai);
            	{$createIndex} \"{$createdIndex}\" ON {$this->getSQLTable($id)} (\"_createdAt\");
            	{$createIndex} \"{$updatedIndex}\" ON {$this->getSQLTable($id)} (\"_updatedAt\");
			";
        }
