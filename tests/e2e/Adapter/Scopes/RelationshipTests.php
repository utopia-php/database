...
trait RelationshipTests
{
    ...
    public function testRelationshipQueryValueLimit(RelationType $type, string $collection, int $limit): void
    {
        ...
        try {
            ...
            foreach ($documents as $document) {
                $key = $collection === 'parents' ? 'children' : 'parents';
                $related = $document->getDocuments($key);
                if ($related === []) {
                    $single = $document->getDocument($key);
                    $related = $single->isEmpty() ? [] : [$single];
                }
    ...
    public function testRelationshipSpatialQueries(): void
    {
        ...
        $restaurants = $database->find('restaurantsSpatial', [
            new Query(Method::Contains, 'supplier.deliveryArea', [[-74.0, 40.75]]),
        ]);
        $this->assertCount(1, $restaurants);
        $this->assertEquals('rest1', $restaurants[0]->getId());
        ...
        $restaurants = $database->find('restaurantsSpatial', [
            new Query(Method::Contains, 'supplier.deliveryRoute', [[-74.0060, 40.7128]]),
        ]);
