...
trait RelationshipTests
{
    ...
    /**
     * A delete inside someone else's transaction is not durable until that transaction
     * commits, so the report waits for it and never arrives if it rolls back.
     */
    public function testDeleteDocumentRelatedCallbackWaitsForOuterCommit(): void
    {
        /** @var Database $database */
        $database = $this->getDatabase();
        ...
        $database->createCollection('outer_parent', permissions: $collectionPermissions, documentSecurity: true);
        $database->createCollection('outer_child', permissions: $collectionPermissions, documentSecurity: true);

        $database->createRelationship(
            collection: 'outer_parent',
            relatedCollection: 'outer_child',
            type: Database::RELATION_ONE_TO_MANY,
            twoWay: true,
            id: 'children',
            twoWayKey: 'parent',
        ...
        foreach (['kept', 'dropped'] as $suffix) {
            $database->createDocument('outer_child', new Document([
                '$id' => 'child_' . $suffix,
                '$permissions' => $documentPermissions,
            ]));

            $database->createDocument('outer_parent', new Document([
                '$id' => 'parent_' . $suffix,
                '$permissions' => $documentPermissions,
                'children' => ['child_' . $suffix],
            ]));
        }

        // Rolled back: the delete never happened, so nothing is reported.
        $reported = [];
        $collect = function (Document $related) use (&$reported) {
            $reported[] = $related->getId();
        };
        ...
        // Committed: reported once, and only after the outer transaction closed.
        $seenInside = null;
        $database->withTransaction(function () use ($database, $collect, &$seenInside, &$reported) {
            $database->deleteDocument('outer_parent', 'parent_kept', $collect);
            $seenInside = $reported;
        });

        $this->assertEquals([], $seenInside);
        $this->assertEquals(['child_kept'], $reported);

        $database->deleteCollection('outer_parent');
        $database->deleteCollection('outer_child');
    }
}
